...
class AlarmManagerTool(llm.Tool):
    ...
    parameters = vol.Schema({
        ...
        vol.Optional(
            "time", 
            description="The timestamp requested for the alarm (e.g., '7:30 AM', '4 PM', '18:45'). Only required if action is 'set'."
        ): str,
        vol.Optional(
            "room", 
            description="The room containing the alarm entity (e.g., 'bedroom' or 'playroom'). Leave blank if room is not given."
        ): str  
    })
    ...
    def _sanitize_time(self, time_str: str) -> str:
        ...
        # andle hour-only entries before am/pm (e.g., "4 pm" -> "4:00 pm")
        if ":" not in clean:
            clean = re.sub(r'\b(\d{1,2})\s*(am|pm)\b', r'\1:00 \2', clean)
