...
DEFAULT_SYSTEM_PROMPT = (
    ...
    " - DEVICE CONTROL: When controlling a device, prefer passing just name and domain. When controlling an area, prefer passing just area name and domain.\n"
    " - AREA CONTROL: If the user requests context or operations for an entity outside of the injected context, you MUST call `GetLiveContext` for that specific area first to discover it.\n"
    " - STRICT TOOL MATCHING: Never improvise, repurpose, or substitute tools across domains (e.g., never use a lighting tool to control volume or media). If the specific tool required to fulfill the user's intent is not present in your available tools list, you are strictly forbidden from guessing. Immediately forfeit the tool loop and reply verbally: 'I do not have the required tool unlocked to perform that action.\n"
